...
//      int[] innArray = {};
        ...
        //리터럴 선언법
        //MemberVO[] mArray3 = new MemberVO[] -> 이게 원래
        MemberVO[] mArray3 = {
                new MemberVO("test1", "홍길동", 21),
                new MemberVO("test2", "박길동", 31),
                new MemberVO("test3", "김길동", 25),
        };

//        mArray[0].setName("홍길동"); //죽는 이유: 지역변수를 초기화 하지 않아서 죽음
//        mArray1[0].setName("홍길동"); //죽는 이유: NullPointerException : null로 초기화 하고 생성 안해서 죽음
//        mArray2[0].setName("홍길동"); //죽는 이유: 배열은 초기화 되었는데, MemberVO를 생성하지 않아서 죽음

        System.out.println(Arrays.toString(mArray2)); //[null, null, null, null, null, null, null, null, null, null]
        // -> 10개의 빈공간만 생성된 배열
        mArray2[0] = new MemberVO(); //0번째 공간에서 Member 생성
//        System.out.println(mArray2[0].getName().contains("홍길동")); //죽는 이우 : name 문자열을 초기화 하지 않아서 죽는다 !
        ...
        //null 체크하는 방법
        if(mArray4 != null){
            System.out.println("mArray4[0]는 null이 아닙니디");
        }
        //null 체크하는 법2
        // -> 안전하게 변수에 접근하는 방법
        if(mArray4[3]  != null){
        ...
        //2. default 생성자를 통해 생성하고 setter로 초기화 하는 방법 -> 비추 !
        // -> 사용되는 이유는 라인을 아래로 늘리려고 사용
        for(int i = 0; i<mArray4.length; i++){
        ...
        //3. 인자 있는 생성자를 사용하는데, 아래로 라인을 늘려야하는 경우
        for(int i = 0; i<mArray4.length; i++){
            String id = "test" + (i + 1);
            String name = "홍길동"+ (i + 1);
            int age = 21 + (i + 1);
